class CardModel {
  ...
  factory CardModel.fromScryfallJson(Map<String, dynamic> json) {
    ...
    final price = json['prices']?['eur'] != null
        ? '${json['prices']['eur']} €'
        : 'N/A';
    final priceFoil = json['prices']?['eur_foil'] != null
        ? '${json['prices']['eur_foil']} €'
        : 'N/A';
    final colorsJson = json['colors'] ?? json['color_identity'];
    final colors = colorsJson is List
        ? colorsJson.map((c) => c.toString()).toList()
        : <String>[];
    final cmc = json['cmc'];
  ...
  factory CardModel.fromMarketplaceJson(Map<String, dynamic> json) {
    return CardModel(
      ...
      artist: json['artist'] ?? '',
      manaCost: json['mana_cost'],
      manaValue: json['cmc'] is num
          ? (json['cmc'] as num).toDouble()
          : double.tryParse('${json['cmc']}'),
  ...
  CardModel mergeWithMarketplaceData(Map<String, dynamic> marketplaceData) {
    ...
    if (price.contains('(Foil:') && price != 'N/A') {
      ...
      finalPrice = price;
    } else if (marketplaceData['price'] != null &&
        marketplaceData['price'] != 'N/A') {
    ...
    return CardModel(
      ...
      imageUrl: imageUrl.isNotEmpty
          ? imageUrl
          : (marketplaceData['image_url'] ?? ''),
      imageNormalUrl: imageNormalUrl,
      expansion: expansion.isNotEmpty
          ? expansion
          : (marketplaceData['set_name'] ?? ''),
      price: finalPrice,
      isFoil: isFoil || (marketplaceData['is_foil'] ?? false),
      condition: condition != 'N/A'
          ? condition
          : (marketplaceData['condition'] ?? 'N/A'),
      username: username != 'N/A'
          ? username
          : (marketplaceData['seller_name'] ?? 'N/A'),
